...
const YourBookingsSection = styled.div`
  margin-bottom: 1.5rem;
  padding: 1rem;
  background: linear-gradient(135deg, rgba(255, 45, 149, 0.1), rgba(255, 45, 149, 0.05));
  border: 1px solid rgba(255, 45, 149, 0.3);
  border-radius: 12px;
`;

const YourBookingsTitle = styled.h3`
  font-family: ${({ theme }) => theme.fonts.heading};
  font-size: 0.85rem;
  font-weight: 700;
...
const YourBookingsList = styled.div`
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
`;

const YourBookingItem = styled.a`
  display: flex;
  align-items: center;
...
const YourBookingTime = styled.span`
  font-family: ${({ theme }) => theme.fonts.heading};
  font-size: 1rem;
  font-weight: 700;
  color: ${({ theme }) => theme.colors.secondary};
...
const YourBookingBadge = styled.span`
  font-family: ${({ theme }) => theme.fonts.body};
  font-size: 0.65rem;
  text-transform: uppercase;
...
const OtherBookingsSection = styled.div`
  margin-bottom: 1.5rem;
  padding: 1rem;
...
const OtherBookingsTitle = styled.h3`
  font-family: ${({ theme }) => theme.fonts.heading};
  font-size: 0.85rem;
  font-weight: 700;
  color: rgba(224, 224, 224, 0.7);
...
const OtherBookingItem = styled.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 0.75rem 1rem;
...
const OtherBookingInfo = styled.div`
  display: flex;
  flex-direction: column;
  gap: 0.25rem;
`;

const OtherBookingTime = styled.span`
  font-family: ${({ theme }) => theme.fonts.heading};
  font-size: 0.9rem;
  font-weight: 700;
...
const OtherBookingDJ = styled.span`
  font-family: ${({ theme }) => theme.fonts.body};
  font-size: 0.75rem;
  color: rgba(224, 224, 224, 0.5);
`;

const OtherBookingBadge = styled.span`
  font-family: ${({ theme }) => theme.fonts.body};
  font-size: 0.6rem;
  text-transform: uppercase;
  letter-spacing: 0.5px;
...
export default function EventDetailPage() {
  ...
  // Get user's booked slots for this event (grouped by consecutive)
  const getUserBookedSets = () => {
    const userSlots = slots
      .filter(s => s.status === 'yours')
      .sort((a, b) => a.slotIndex - b.slotIndex);
    
    if (userSlots.length === 0) return [];
    
    // Group consecutive slots
    const sets: { slots: TimeSlot[]; startTime: Date; endTime: Date }[] = [];
    let currentSet: TimeSlot[] = [];
    
    for (const slot of userSlots) {
      if (currentSet.length === 0) {
        currentSet.push(slot);
      } else {
        const lastSlot = currentSet[currentSet.length - 1];
        if (slot.slotIndex === lastSlot.slotIndex + 1) {
          currentSet.push(slot);
        } else {
          // Save current set and start new one
          sets.push({
            slots: currentSet,
            startTime: currentSet[0].startTime,
            endTime: currentSet[currentSet.length - 1].endTime,
          });
          currentSet = [slot];
        }
      }
  ...
  const userBookedSets = getUserBookedSets();

  // Get other users' booked slots (grouped by DJ and consecutive)
  const getOtherBookedSets = () => {
    const otherSlots = slots
      .filter(s => s.status === 'booked' && s.djName)
      .sort((a, b) => a.slotIndex - b.slotIndex);
    
    if (otherSlots.length === 0) return [];
    
    // Group consecutive slots by the same DJ
    const sets: { slots: TimeSlot[]; startTime: Date; endTime: Date; djName: string }[] = [];
    let currentSet: TimeSlot[] = [];
    let currentDJ: string | undefined;
    
    for (const slot of otherSlots) {
      if (currentSet.length === 0) {
        currentSet.push(slot);
        currentDJ = slot.djName;
      } else if (
        slot.djName === currentDJ && 
        slot.slotIndex === currentSet[currentSet.length - 1].slotIndex + 1
      ) {
        // Same DJ and consecutive
        currentSet.push(slot);
      } else {
        // Save current set and start new one
        sets.push({
  ...
  const otherBookedSets = getOtherBookedSets();
  ...
  return (
    <AppLayout title={`${event.title} | DJ Tap-In Queue`}>
      <PageContainer>
        ...
        {/* Show user's existing bookings for this event */}
        {userBookedSets.length > 0 && (
          <YourBookingsSection>
            <YourBookingsTitle>
              🎧 Your Set{userBookedSets.length > 1 ? 's' : ''}
            </YourBookingsTitle>
            <YourBookingsList>
              {userBookedSets.map((set, idx) => (
                <Link key={idx} href="/bookings" passHref legacyBehavior>
                  <YourBookingItem>
                    <YourBookingTime>
                      {formatTime(set.startTime.toISOString())} - {formatTime(set.endTime.toISOString())}
                    </YourBookingTime>
                    <YourBookingBadge>
                      {set.slots.length} slot{set.slots.length > 1 ? 's' : ''}
                    </YourBookingBadge>
                  </YourBookingItem>
                </Link>
              ))}
        ...
        {/* Show other DJs' bookings for this event */}
        {otherBookedSets.length > 0 && (
          <OtherBookingsSection>
            <OtherBookingsTitle>
              Booked Sets
            </OtherBookingsTitle>
            {otherBookedSets.map((set, idx) => (
              <OtherBookingItem key={idx}>
                <OtherBookingInfo>
                  <OtherBookingTime>
                    {formatTime(set.startTime.toISOString())} - {formatTime(set.endTime.toISOString())}
                  </OtherBookingTime>
                  <OtherBookingDJ>{set.djName}</OtherBookingDJ>
                </OtherBookingInfo>
                {set.slots.length > 1 && (
                  <OtherBookingBadge>
